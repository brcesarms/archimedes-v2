# ⚖️ Decisão de Arquitetura (ADR 001) — Python no Orquestrador, PowerShell no Cliente

> **Data:** 2026-09-11 · **Status:** Decidido ✅ · **Tipo:** Decisão de Arquitetura (ADR)

## 🎯 Resumo

**Decisão:** NÃO instalar Python nas máquinas dos clientes. Manter o split **orquestrador em Python (lado do Archimedes)** + **agentes em PowerShell nativo (lado do cliente)**.

Essa é a arquitetura padrão de mercado (padrão "controller + agent" — idêntico ao Ansible: Python no controller, comandos nativos no alvo).
...
Análise de arquitetura concluída com foco em confiabilidade e boas práticas.
...
| 🔄 **Manutenção** | Quem atualiza o Python do cliente depois da entrega? PowerShell recebe update sozinho pelo sistema operacional |

---

## ✅ O padrão correto (arquitetura do projeto)

```text
🧠 Archimedes (Python / Pyinfra / Paramiko)  →  🖥️ Máquina do cliente (PowerShell)
    Lógica pesada e orquestração                    Scripts simples, atômicos e nativos
    roda onde VOCÊ controla                         rodam uma vez e somem
```

| Camada | Linguagem | Onde roda | Função |
| :--- | :--- | :--- | :--- |
| **Orquestrador** | 🐍 Python / Pyinfra | Máquina do Archimedes (GEEKOM/Alienware) | Conecta SSH/SFTP, dispara comandos, parseia telemetria, gera manifesto |
| **Agente** | 🪟 PowerShell 5.1 | Máquina do cliente | Inventário, backup robocopy, setup SSH, ajustes — roda sob demanda |
...
| Cliente **JÁ tem Python** (dev/analista) | Aproveitar o ambiente pré-existente — sem instalar nada novo |
...
- [Projeto Bancada (archimedes-operator)](https://github.com/brcesarms/archimedes-operator)
- [Runbook: Proxmox GEEKOM — Setup e Acesso SSH à VM Windows](../runbooks/proxmox-geekom-vm-windows.md)
- [AGENTS.md](../../AGENTS.md)
