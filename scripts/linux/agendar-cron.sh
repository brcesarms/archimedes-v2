...
case "$MODE" in
  --install | -i)
    ...
    [ -f "$SYSTEMD_DIR/logs-rotator.service" ] || {
      log "❌ logs-rotator.service não encontrado"
      exit 1
    }
    [ -f "$SYSTEMD_DIR/logs-rotator.timer" ] || {
      log "❌ logs-rotator.timer não encontrado"
      exit 1
    }
    [ -f "$SYSTEMD_DIR/backup-logs.service" ] || {
      log "❌ backup-logs.service não encontrado"
      exit 1
    }
    [ -f "$SYSTEMD_DIR/backup-logs.timer" ] || {
      log "❌ backup-logs.timer não encontrado"
      exit 1
    }
